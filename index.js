...
function matkaleRegistreerumine(req, res) {
  ...
  if (valitudMatkaIndeks >= 0 && valitudMatkaIndeks < koikMatkad.length) {
    return res.render("pages/registreerimine", {
      matk: valitudMatkaAndmed,
      matkaIndeks: valitudMatkaIndeks,
    });
  } else {
  ...
  client.connect((err) => {
    if (err) {
      ...
      collection.insertOne(registreerunu, (err) => {
        ...
        return res.render("pages/kinnitus", {
          matk: valitudMatk,
          isikNimi: registreerunu.nimi,
        });
      });
    }
  });
...
function matkaleRegistreerunud(req, res) {
  ...
  client.connect((err) => {
      ...
      const collection = client
        .db(andmebaas)
        .collection("matkaklubi_" + andmebaas + "_registreerumised");
      collection.find( {matk: 0}).toArray( 
...
express()
  ...
  .get("/uudised", function (req, res) {
    return res.render("pages/uudised");
  })
  .get("/kontakt", function (req, res) {
    return res.render("pages/kontakt");
  })
  .get("/registreerimine", function (req, res) {
    return res.render("pages/registreeru");
  })
